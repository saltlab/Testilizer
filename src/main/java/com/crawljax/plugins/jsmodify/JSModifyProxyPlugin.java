...
import com.crawljax.plugins.diffcrawler.JSCodeChangeAnalyzer;
...
public class JSModifyProxyPlugin extends ProxyPlugin {
	...
	private static final Logger LOG = LoggerFactory.getLogger(JSModifyProxyPlugin.class);
	...
	private boolean shouldModify(String name) {
		...
		for (String pattern : excludeFilenamePatterns) {
			if (name.matches(pattern)) {
				LOG.info("Not modifying response for " + name);
				return false;
			}
		}

		LOG.info("Modifying response for " + name);
	...
	private synchronized String modifyJS(String input, String scopename) {
		...
		try {
			...
			JSCodeChangeAnalyzer.setJSName(jsName);

	
			/**
			 * TODO: Analysing inline JavasSript code changes
			 */
			if (scopename.contains("script")){
				//System.out.println("scopename : " + getJSName(scopename));
				JSCodeChangeAnalyzer.analyseInline(jsName, input, jsInTag);
			}else if (foundTagsWithJS){
				JSCodeChangeAnalyzer.analyseInline("main_html", "", jsInTag);
			}
...
//			}
			...
			/* recurse through AST and analyze the code for changes*/
			ast.visit(modifier);
			...
			try {
				...
				fop.flush();
				LOG.info("Printing instrumented code to {}", file.getName());
				fop.close();
			}
			catch (IOException ioe) {
				LOG.info("Could not write the instrumented file into disk!");
			}
			...
			// Add to the list of instrumented JS
			...
			LOG.warn("Unable to instrument. This might be a JSON response sent"
			        + " with the wrong Content-Type or a syntax error.");
		} catch (IllegalArgumentException iae) {
			LOG.warn("Invalid operator exception catched. Not instrumenting code.");
		}
	...
	private Response createResponse(Response response, Request request) {
		...
		if (request.getURL().toString().contains("?thisisanexecutiontracingcall")) {
			LOG.info("Execution trace request " + request.getURL().toString());
		...
		if (type != null && type.contains("javascript")) {
			...
			try {
				Document dom = DomUtils.asDocument(new String(response.getContent()));
